...
class MainWindow(QtWidgets.QMainWindow, WindowMixin):
    ...
    def __init__(self, config=None, filename=None, output=None):
        ...
        zoomIn = action('Zoom &In', functools.partial(self.addZoom, 100),
                        shortcuts['zoom_in'], 'zoom-in',
                        'Increase zoom level', enabled=False)
        zoomOut = action('&Zoom Out', functools.partial(self.addZoom, -100),
        ...
        # window_y, window_x = self.imageData.shape
    ...
    def zoomRequest(self, delta, pos):
        ...
        ### canvas width new and old
        ## change the deepzoom image ??
        ...
        if canvas_width_old != canvas_width_new:
            ...
            # print('x: %f, shift: %f' % pos.x(), x_shift)
            # print('y: %f, shift: %f' % pos.y(), y_shift)
    ...
    def loadFile(self, filename=None):
        ...
        if QtCore.QFile.exists(label_file) and \
            ...
            try:
                ...
                # im = self.imageData.convert("RGBA")
                # data = im.tobytes("raw", "RGBA")

                # if QtGui.QImage(data, im.size[0], im.size[1], QtGui.QImage.Format_RGBA8888).isNull():
            ...
            self.imagePath = os.path.join(os.path.dirname(label_file),self.labelFile.imagePath)
            ...
            self.imageData, self.dzImage = read(filename, None)
            if self.imageData is not None:
                # the filename is image not JSON
                self.imagePath = filename
            self.labelFile = None

            if str(type(self.imageData)) == "<class 'bytes'>":
                image = QtGui.QImage.fromData(self.imageData)
            elif str(type(self.imageData)) == "<class 'PIL.Image.Image'>":
                im = self.imageData.convert("RGBA")
                data = im.tobytes("raw", "RGBA")
                image = QtGui.QImage(data, im.size[0], im.size[1], QtGui.QImage.Format_RGBA8888)
            elif str(type(self.imageData)) == "<class 'numpy.ndarray'>":
                # qim = QImage(im.data, im.shape[1], im.shape[0], im.strides[0], QImage.Format_Indexed8)
                # image = QtGui.QImage(self.imageData, self.imageData.shape[0], self.imageData.shape[1],
                #                     QtGui.QImage.Format_Grayscale8)
                image = qnda.array2qimage(self.imageData)
        ...
        if self.dzImage is not None:
            self.dzLevel = self.dzImage.level_count
            initLevel = 17 # 확대 가능한 정도 level

            tiles_x, tiles_y = self.dzImage.level_tiles[initLevel]
            print(tiles_x, tiles_y) # grid 표시
            img = self.dzImage.get_tile(level=initLevel, address=(10, 10))
            im = img.convert("RGBA")
            data = im.tobytes("raw", "RGBA")
            image = QtGui.QImage(data, im.size[0], im.size[1], QtGui.QImage.Format_RGBA8888)
        ...
        QImg = QImg.scaled(600, 500, Qt.KeepAspectRatio)
        self.canvas.loadPixmap(QImg)
    ...
    def openFile(self, _value=False):
        ...
        formats = ['*.svs', '*.dcm']
        forms = ['*.{}'.format(fmt.data().decode())
                   for fmt in QtGui.QImageReader.supportedImageFormats()]
        formats.extend(f for f in forms)
    ...
    def scanAllImages(self, folderPath):
        extensions = ['.%s' % 'svs', 'dcm']
        extensions.extend(['.%s' % fmt.data().decode("ascii").lower()
                      for fmt in QtGui.QImageReader.supportedImageFormats()])
        images = []
    ...
    try:
        if filename[-4:] == '.svs':
            ...
            if slide.level_count == 1:
                return slide.get_thumbnail(slide.associated_images['thumbnail'].size), None
                # img = slide.get_thumbnail(slide.associated_images['thumbnail'].size)
                #
                # im = img.convert("RGBA")
                # data = im.tobytes("raw", "RGBA")
                # return QtGui.QImage(data, im.size[0], im.size[1], QtGui.QImage.Format_RGBA8888), None
            else:
                deepzoom = DeepZoomGenerator(slide)
                return slide.get_thumbnail(slide.associated_images['thumbnail'].size), deepzoom
                # img = slide.get_thumbnail(slide.associated_images['thumbnail'].size)
                # # img = deepzoom.get_tile(level=13, address=(0,1))
                # im = img.convert("RGBA")
                # data = im.tobytes("raw", "RGBA")
                #
                # # deepzoom = deepzoomgeneration(slide)
                # return QtGui.QImage(data, im.size[0], im.size[1], QtGui.QImage.Format_RGBA8888), deepzoom
        elif filename[-4:] == ".dcm":
            input = dicom.read_file(filename)
            img = input.pixel_array
            mini = img.min()
            maxi = img.max()
